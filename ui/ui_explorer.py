...
class explorer:
  ...
  def draw():
    ...
    ui.img.draw_rectangle((0,0,240,240), fill=True, color=(50, 50, 50))

    ui.img.draw_string(10, 5, str(explorer.get_path(explorer.paths)), scale=1)

    ui.img.draw_string(10, 225, str(explorer.info), scale=1)

    list_size = len(explorer.files)

    if list_size > 4:
        tmp = (150, 150, 150)
        ui.img.draw_rectangle((65, 25, 200, 25), fill=True, color=(75, 75, 75))
        ui.img.draw_string(70, 30, str(explorer.files[-2]), scale=1, color=tmp)
        ui.img.draw_rectangle((65, 195, 200, 25), fill=True, color=(75, 75, 75))
        ui.img.draw_string(70, 200, str(explorer.files[+2]), scale=1, color=tmp)

    if list_size > 2:
        tmp = (200, 200, 200)
        ui.img.draw_rectangle((45, 55, 200, 35), fill=True, color=(100, 100, 100))
        ui.img.draw_string(50, 60, str(explorer.files[-1]), scale=2, color=tmp)
        ui.img.draw_rectangle((45, 155, 200, 35), fill=True, color=(100, 100, 100))
        ui.img.draw_string(50, 160, str(explorer.files[+1]), scale=2, color=tmp)

    if list_size > 0:
        tmp = (255, 255, 255)
        #ui.img.draw_ellipse(120, 120, 200, 25, 0, color=(226, 205, 223), thickness=2, fill=True)
        ui.img.draw_rectangle((20, 95, 220, 55), fill=True, color=(0x5b, 0x86, 0xec))
        ui.img.draw_string(30, 105, str(explorer.files[0]), scale=3, color=tmp)
