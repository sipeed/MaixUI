...
class Struct:
    ...
    def __get__(self, obj, objtype=None):
        obj.i2c.writeto(obj.dev_addr, self.buffer)
        obj.i2c.readfrom_into(obj.dev_addr, self.buffer)
        return struct.unpack_from(self.format, memoryview(self.buffer)[1:])

    def __set__(self, obj, value):
        struct.pack_into(self.format, self.buffer, 1, *value)
        obj.i2c.writeto(obj.dev_addr, buf)
...
class UnaryStruct:
    ...
    def __get__(self, obj, objtype=None):
        ...
        buf[0] = self.address
        obj.i2c.writeto(obj.dev_addr, buf)
        obj.i2c.readfrom_into(obj.dev_addr, buf)
...
class MSA301Error(Exception):
    def __init__(info):
        print("Error: MSA301 Unkown Error!", info);


class NotFoundError(MSA301Error):
    def __init__(info):
        print("Error: Unable connect MSA301!", info);
    # pass


class OutOfRange(MSA301Error):
    pass
...
class RWBits:
    ...
    def __get__(self, obj, objtype=None):
        self.buffer = bytearray(obj.i2c.readfrom(obj.dev_addr, len(self.buffer)))
        #obj.i2c.readfrom_into(obj.dev_addr, self.buffer)
        print(self.buffer)
...
class MAS301:
    ...
    def __init__(self, i2c_bus=None, i2c_addr=0x26):
        if i2c_bus is None:
            try:
                self.i2c = I2C(I2C.I2C1, freq=100*1000, sda=31, scl=30)
            except Exception:
                raise MSA301Error("Unable to init I2C0 as Master")
        else:
            self.i2c = i2c_bus
        ...
        #print("i2c_dev list:" + str(dev_list), self.dev_addr in dev_list)
        #if self.dev_addr not in dev_list:
            #raise NotFoundError("Error: Unable connect MSA301!")
    ...
    def __write_reg(self, reg_address, value):
        self.bus.writeto_mem(
            self.dev_addr, reg_address, value, mem_size=8)

    def __read_reg(self, reg_address):
        self.bus.writeto(self.dev_addr, bytes([reg_address]))
        return (self.bus.readfrom(self.dev_addr, 1))[0]
    ...
    def acceleration(self):
        ...
        x_acc = ((x >> 2) / scale) * MAS301._STANDARD_GRAVITY
        y_acc = ((y >> 2) / scale) * MAS301._STANDARD_GRAVITY
        z_acc = ((z >> 2) / scale) * MAS301._STANDARD_GRAVITY
...
if __name__ == "__main__":
    ...
    while True:
        msa301.acceleration
        #print("%f %f %f" % msa301.acceleration)
        time.sleep(0.5)
